"""OpenAI provider para extracao de documentos"""
...
class OpenAIProvider(BaseProvider):
    """Provider OpenAI GPT-4 Vision para extracao de documentos"""
    ...
    async def extract_from_image(self, image_content: bytes, mime_type: str) -> dict:
        """Extrai dados de uma imagem usando OpenAI GPT-4 Vision"""
        import httpx

        api_key = settings.openai_api_key
        if not api_key:
            return {"items": [], "error": "OPENAI_API_KEY nao configurada"}
    ...
    async def extract_from_pdf(
        ...
        """OpenAI nao suporta PDF direto - retorna None para usar fallback"""
        return None
